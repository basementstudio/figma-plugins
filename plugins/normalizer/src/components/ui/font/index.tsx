  ...
  return (
    ...
    >
      <div
        ...
        style={{
          fontFamily: `${text.fontName?.family || 'sans-serif'}, Geist, sans-serif`,
      ...
      <span className="text-xs leading-none text-gray-950 text-ellipsis max-w-full overflow-hidden max-h-3 w-full text-left whitespace-nowrap">
        {text.fontName?.family || 'Unknown'} - {text.fontName?.style || 'Regular'}
      </span>
      {!options?.hideDetails && (
      ...
      <UsesTag uses={text.uses?.length || 0} timesSmall={options?.timesSmall} />
    </div>
  );
}
