...
import * as place from '../utils/place';
  ...
  if (data.other.type === 'TEXT' || data.other.type === 'FRAME' || data.other.type === 'GROUP') {
    if (firstRender) {
      let textnode: TextNode[] =
        data.other.type === 'TEXT' ? [data.other] : (data.other.findAll((e) => e.type === 'TEXT') as TextNode[]);

      for (const find of textnode) {
        for (let i = 0; i < find.characters.length; i++) {
          const getFontName = find.getRangeFontName(i, i + 1) as FontName;
          await figma.loadFontAsync(getFontName);
...
figma.ui.on('message', async (msg) => {
  ...
  if (msg.type === 'initial-link') {
    ...
    // make a new group
    const group: GroupNode = figma.group([clone2], data.curve.parent);
...
figma.showUI(__html__, { width: 600, height: 480 });
