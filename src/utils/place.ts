 ...
 * @param curve
 */
const place = (object: SceneNode, point: Point, options: SettingsData, curve: VectorNode) => {
 ...
 * @param group
 */
export const text2Curve = async (node: TextNode, pointArr: Array<Point>, data: LinkedData, group: GroupNode) => {
  const newNodes: SceneNode[] = [];

  const options: SettingsData = data.setting;
  // convert text into each letter individually
  ...
  for (let i = 0; i < charArr.length; i++) {
    ...
    // Can be used to set the font size of the text
    // node.fontSize = options.textSize;
    // letter.fontSize = options.textSize;
    ...
    pass.spacing =
      pass.spacing +
      letter.width * options.horizontalAlign +
      prevletter * (1 - options.horizontalAlign) +
      options.spacing;
 ...
 * @param group
 */
export const object2Curve = (node: SceneNode, pointArr: Array<Point>, data: LinkedData, group: GroupNode) => {
  const newNodes: SceneNode[] = [];
  var options: SettingsData = data.setting;
