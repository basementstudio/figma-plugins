 ...
 */
const casteljau = (curve: Array<Point>, t: number, setting: SettingsData, calcRot: boolean = false): Point[] => {
  let arr: Point[] = [];
  ...
  for (var c = 0; c < curve.length - 1; c++) {
    ...
    if (calcRot && setting.rotCheck) {
      ...
      let angle = Math.atan((curve[c + 1].x - curve[c].x) / (curve[c + 1].y - curve[c].y)) * (180 / Math.PI);
 ...
 */
const pointOnCurve = (curve: Array<Point>, setting: SettingsData, totalDist: number): Point[] => {
  let finalarr: Point[] = [];
 ...
 * @param rotation
 */
export const allPoints = (svgData: string, setting: SettingsData): Point[] => {
  let pointArr: Point[] = [];
